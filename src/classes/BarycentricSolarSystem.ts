...
const collectOrbitPaths = (objects: CelestialObjectModel[], time: number) => {
  ...
  const sampleTimes = [
    10000000, 100000000, 500000000, 1000000000, 10000000000, 100000000000
  ]
  const paths = new Array(objects.length)
    .fill(0)
    .map(() => [] as Array<{ x: number; y: number; z: number }>)
...
const getSnapshotData = (objects: CelestialObjectModel[], time: number) => {
  ...
  planets.forEach((curr) => {
    ...
    const {
      north: { x: rx, y: ry, z: rz },
      spin
    } = Astronomy.RotationAxis(curr.name as Body, astronomyTime)
  ...
  const {
    north: { x: rx, y: ry, z: rz },
    spin
  } = Astronomy.RotationAxis(sun.name as Body, astronomyTime)
